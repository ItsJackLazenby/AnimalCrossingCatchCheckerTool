...
export default function Home() {
  ...
  function handleClick() {
    ...
    const filtered = filterAllCritters(date, "NH");
    setResults(filtered);
  }
...
function CategoryColumn({ title, items }: { title: string; items: any[] }) {
  const imageBasePath = "/images/"; // Place your Critterpedia images in public/images/
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {items.map((c) => {
          ...
          return (
            <div
              key={c["#"]}
              className="p-4 bg-zinc-100 dark:bg-zinc-900 rounded-lg border border-zinc-300 dark:border-zinc-700 hover:shadow-md transition"
            >
